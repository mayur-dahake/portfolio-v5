...
export function sanitizeFormData(data, fieldMap) {
  ...
  for (const [key, type] of Object.entries(fieldMap)) {
    ...
    if (type === 'array')     result[key] = (result[key] || []).map(v => sanitizeText(v)).filter(Boolean);
  }
  return result;
}
...
export function validateContactForm({ name, email, message }) {
  ...
  else if (cleanName.length > 100) errors.name = 'Name must be 100 characters or fewer.';

  const cleanEmail = sanitizeText(email).toLowerCase();
  if (!cleanEmail) errors.email = 'Email is required.';
  else if (!isValidEmail(cleanEmail)) errors.email = 'Please enter a valid email address (e.g. you@example.com).';

  const cleanMessage = sanitizeMultiline(message);
  if (!cleanMessage) errors.message = 'Message is required.';
  else if (cleanMessage.length < 10) errors.message = 'Message must be at least 10 characters.';
  else if (cleanMessage.length > 2000) errors.message = `Message is too long (${cleanMessage.length}/2000 characters).`;
  else if (containsSpam(cleanMessage)) errors.message = 'Message contains disallowed content.';
