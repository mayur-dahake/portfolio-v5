      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 p-5 md:p-7">
          <div className="flex-1 min-w-0">
            ...
            {/* Company */}
            <p
              className={`text-base font-semibold mt-1 ${darkMode ? "text-white/60" : "text-black/60"}`}
            >
              {exp.company}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-3">
              <span
                className={`flex items-center gap-1.5 text-xs font-mono ${darkMode ? "text-white/30" : "text-black/30"}`}
              >
                <Calendar className="w-3 h-3" />
                {formatDate(exp.startDate)} —{" "}
                {exp.isCurrent ? (
                  <span className="text-[#ff0080]">PRESENT</span>
                ) : (
          ...
          <motion.div
            animate={{
              ...
              borderColor:
                hovered || isExpanded ? "#ff0080" : "rgba(255,255,255,0.2)"
            }}
        ...
        <AnimatePresence initial={false}>
          {isExpanded && (
            ...
            >
              <div
                className={`px-5 md:px-7 pb-7 border-t grid md:grid-cols-2 gap-6 pt-6 ${darkMode ? "border-white/10" : "border-black/10"}`}
              >
                {/* Description */}
                {exp.description && (
                  <motion.p
                ...
                {exp.techStack && exp.techStack.length > 0 && (
                  ...
                  >
                    <p
                      className={`text-[10px] font-mono tracking-widest mb-3 ${darkMode ? "text-white/30" : "text-black/30"}`}
                    >
                      TECHNOLOGIES
                    </p>
