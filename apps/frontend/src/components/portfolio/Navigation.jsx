    ...
    <>
      ...
      <motion.nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled 
            ? darkMode ? 'bg-[#0a0a0a]/90 backdrop-blur-md' : 'bg-white/90 backdrop-blur-md' 
            : ''
        }`}
        initial={{ y: -100 }}
        animate={{ y: 0 }}
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <span className="text-[10px] font-mono text-[#ff0080] mr-1">{item.num}</span>
                  {item.label}
                  {isActive && (
                    <motion.div
                      layoutId="nav-active"
            ...
            <button
              ...
              className={`w-10 h-10 flex items-center justify-center transition-colors ${
                darkMode 
                  ...
                  : scrolled ? 'text-black/50 hover:text-[#ff0080]' : 'text-white/50 hover:text-[#ff0080]'
              }`}
            >
          ...
          >
            ...
            <div className="flex-1 flex flex-col justify-center px-6">
              {navItems.map((item, index) => (
                ...
                >
                  <span className="text-xs font-mono text-[#ff0080] block mb-1">{item.num}</span>
