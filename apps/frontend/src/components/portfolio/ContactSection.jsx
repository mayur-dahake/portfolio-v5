...
import { Mail, Github, Linkedin, ArrowUpRight, Copy, Send, CheckCircle, AlertCircle } from 'lucide-react';
...
import { sanitizeText, sanitizeMultiline, validateContactForm } from "@/components/utils/sanitize";
...
export default function ContactSection({ profile, darkMode }) {
  ...
  const handleSubmit = async (e) => {
    ...
    recordSubmission();
    try {
      const subject = encodeURIComponent(`Portfolio Contact: Message from ${clean.name}`);
  ...
  return (
    <section id="contact" className={`min-h-screen flex items-center py-16 md:py-32 px-4 md:px-12 lg:px-24 relative overflow-hidden ${darkMode ? 'bg-[#111]' : 'bg-[#f5f5f0]'}`}>
      {/* Large decorative element */}
      <div className={`absolute top-1/2 right-0 -translate-y-1/2 translate-x-1/2 w-[60vw] h-[60vw] rounded-full border pointer-events-none ${darkMode ? 'border-white/5' : 'border-black/5'}`} />
      <div className={`absolute top-1/2 right-0 -translate-y-1/2 translate-x-1/2 w-[40vw] h-[40vw] rounded-full border pointer-events-none ${darkMode ? 'border-white/5' : 'border-black/5'}`} />
      ...
      <div className="max-w-6xl mx-auto relative z-10 w-full">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <span className={`text-xs font-mono tracking-widest ${darkMode ? 'text-white/40' : 'text-black/40'}`}>005</span>
          <div className={`w-16 h-px ${darkMode ? 'bg-white/20' : 'bg-black/20'}`} />
          <span className={`text-xs font-mono tracking-widest ${darkMode ? 'text-white/40' : 'text-black/40'}`}>CONTACT</span>
        </motion.div>
        ...
        <div className="mb-20">
          ...
          >
            LET'S CREATE
            <br />
            <span className={`text-transparent ${darkMode ? '[-webkit-text-stroke:2px_white]' : '[-webkit-text-stroke:2px_black]'}`}>SOMETHING</span>
            <br />
        ...
        {profile?.email && (
          ...
          >
            <p className={`text-xs font-mono tracking-widest mb-4 ${darkMode ? 'text-white/40' : 'text-black/40'}`}>SAY HELLO</p>
            <a
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <p className={`text-xs font-mono tracking-widest mb-6 ${darkMode ? 'text-white/40' : 'text-black/40'}`}>SEND A MESSAGE</p>
          ...
          {submitStatus === 'rate_limited' && (
            <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/30 flex items-center gap-3">
              ...
              <p className="text-yellow-400 text-sm">Too many submissions. Please wait a few minutes before trying again.</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4 max-w-2xl">
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="contact-name" className={`text-[10px] font-mono tracking-widest ${darkMode ? 'text-white/30' : 'text-black/30'}`}>NAME *</label>
              <input
                id="contact-name"
                type="text"
                ...
                className={`px-4 py-3 text-sm font-mono border bg-transparent outline-none focus:border-[#ff0080] transition-colors ${
                  fieldErrors.name
                    ? 'border-red-500'
                    : darkMode ? 'border-white/20 text-white placeholder:text-white/30' : 'border-black/20 text-black placeholder:text-black/30'
                }`}
              />
              {fieldErrors.name && (
            ...
            {/* Email */}
            <div className="flex flex-col gap-1">
              <label htmlFor="contact-email" className={`text-[10px] font-mono tracking-widest ${darkMode ? 'text-white/30' : 'text-black/30'}`}>EMAIL *</label>
              <input
                id="contact-email"
                type="email"
            ...
            <div className="md:col-span-2 flex flex-col gap-1">
              <label htmlFor="contact-message" className={`text-[10px] font-mono tracking-widest ${darkMode ? 'text-white/30' : 'text-black/30'}`}>MESSAGE *</label>
              <textarea
                id="contact-message"
              ...
              <div className="flex items-center justify-between">
                {fieldErrors.message ? (
                  <p className="text-red-400 text-xs font-mono flex items-center gap-1">
                    <AlertCircle className="w-3 h-3 flex-shrink-0" /> {fieldErrors.message}
                  </p>
                ) : <span />}
                <span className={`text-xs font-mono ml-auto ${formData.message.length > 1800 ? 'text-yellow-400' : darkMode ? 'text-white/20' : 'text-black/30'}`}>
        ...
        {socialLinks.length > 0 && (
          ...
          >
            <p className={`text-xs font-mono tracking-widest mb-6 ${darkMode ? 'text-white/40' : 'text-black/40'}`}>ELSEWHERE</p>
