...
export default function SkillsSection({ skills, darkMode }) {
  ...
  return (
    <section id="skills" className={`py-16 md:py-32 px-4 md:px-12 lg:px-24 relative overflow-hidden ${darkMode ? 'bg-[#0a0a0a]' : 'bg-[#f0f0ea]'}`}>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <span className={`text-xs font-mono tracking-widest ${darkMode ? 'text-white/40' : 'text-black/40'}`}>004</span>
          <div className={`w-16 h-px ${darkMode ? 'bg-white/20' : 'bg-black/20'}`} />
          <span className={`text-xs font-mono tracking-widest ${darkMode ? 'text-white/40' : 'text-black/40'}`}>SKILLS & TOOLS</span>
        </motion.div>
        ...
        <div className="space-y-5">
          {categories.map((category, catIndex) => (
            ...
            >
              <h3 className="text-[10px] font-mono text-[#ff0080] tracking-widest mb-3" aria-label={`${category} skills`}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {groupedSkills[category].map((skill, skillIndex) => (
                  ...
                  >
                    <div className={`px-2.5 py-1.5 border hover:border-[#ff0080] hover:bg-[#ff0080]/10 transition-all duration-300 cursor-default flex items-center gap-1.5 ${darkMode ? 'border-white/20' : 'border-black/20'}`}>
                      {/* Skill Icon */}
                      <span className={`group-hover:text-[#ff0080] transition-colors [&>svg]:w-3 [&>svg]:h-3 ${darkMode ? 'text-white/60' : 'text-black/50'}`}>
                        {skillIcons[skill.name] || (
                          <div className={`w-3 h-3 rounded flex items-center justify-center text-[8px] font-bold ${darkMode ? 'bg-white/10' : 'bg-black/10'}`}>
                            {skill.name?.charAt(0)}
                          </div>
                        )}
                      </span>
                      <span className={`text-xs font-medium group-hover:text-[#ff0080] transition-colors ${darkMode ? 'text-white' : 'text-black'}`}>
                        {skill.name}
                      </span>
                      {skill.proficiency && (
                        <span className={`text-[9px] font-mono px-1 py-0.5 rounded leading-none ${
                          skill.proficiency >= 80
                            ...
                            : skill.proficiency >= 55
                              ? darkMode ? 'bg-white/10 text-white/50' : 'bg-black/10 text-black/50'
                              : darkMode ? 'bg-white/5 text-white/30' : 'bg-black/5 text-black/30'
                        }`}>
                          {skill.proficiency >= 80 ? 'ADV' : skill.proficiency >= 55 ? 'INT' : 'FAM'}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-black/90 border border-white/10 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-20 w-48 text-center">
                      <p className="text-xs text-white/70">
                        {skillDescriptions[skill.name] || `Expert in ${skill.name}`}
                      </p>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <p className={`text-[12vw] md:text-[8vw] font-black leading-none ${darkMode ? 'text-white/[0.03]' : 'text-black/[0.03]'}`}>
            ALWAYS LEARNING
          </p>
        </motion.div>
      </div>
