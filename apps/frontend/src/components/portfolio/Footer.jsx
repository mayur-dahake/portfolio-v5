...
export default function Footer({ profile, darkMode }) {
  ...
  return (
    <footer
      className={`py-8 px-6 md:px-12 lg:px-24 border-t ${darkMode ? "bg-[#0a0a0a] border-white/10" : "bg-[#f5f5f0] border-black/10"}`}
    >
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        <p
          className={`text-xs font-mono ${darkMode ? "text-white/30" : "text-black/30"}`}
        >
        ...
        <p
          className={`text-xs font-mono ${darkMode ? "text-white/30" : "text-black/30"}`}
        >
          BUILT WITH <span className="text-[#ff0080]">♥</span> AND LOTS OF
          COFFEE
        </p>
